...
def diagnose_recent(limit: int = 20) -> dict[str, Any]:
    ...
    for image_id in image_ids:
        try:
            result = diagnose_face_candidates(
                image_id,
                top_n=3,
                scan_if_missing=True,
            )
            rows.append(result)
            reasons[str(result.get("overall_reason") or "不明")] += 1
...
def _single_embed(result: dict[str, Any]) -> discord.Embed:
    ...
    for index, face in enumerate(result.get("faces", [])[:8], 1):
        ...
        candidate_lines = [
            (
                f"{i}. {c['person_name']} "
                f"**{float(c['confidence']) * 100:.1f}%**"
                + (
                    f"（顔 {float(c.get('face_similarity', 0)) * 100:.1f}%）"
                    if c.get("face_similarity") is not None
                    else ""
                )
            )
        ...
        safe_add_field(
            embed,
            name=f"顔{index}（face_id={face['face_id']}）",
            value=value,
            inline=False,
        )
    ...
    if len(result.get("faces", [])) > 8:
        embed.set_footer(
            text=f"顔は全{len(result['faces'])}件です。表示は先頭8件まで。OpenAI APIは使用していません。"
        )
    else:
        embed.set_footer(
            text="診断は保存済みDBとローカル顔特徴量だけで実行します。OpenAI APIは使用しません。"
        )
    return embed
...
def _recent_embed(report: dict[str, Any]) -> discord.Embed:
    ...
    embed.description = (
        f"対象 **{total}枚** / 候補しきい値 **{float(report['threshold']) * 100:.1f}%**\n"
    ...
    for item in report["images"][:20]:
        ...
        best_text = (
            f" / 最高 {best_name} {best_score * 100:.1f}%"
            if best_score > 0
            else ""
        )
        detail_lines.append(
            f"`{item['image_id']}` {item['overall_reason']}{best_text}"
        )
    ...
    embed.set_footer(
        text="候補0件でも、最高候補としきい値差を確認できます。"
    )
    return embed


class FaceDiagnosticIdModal(
    discord.ui.Modal,
    title="顔候補診断：画像ID指定",
):
    image_id = discord.ui.TextInput(
        label="画像ID",
        placeholder="例: 85770",
        max_length=20,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            await interaction.followup.send(
                f"⚠️ 診断に失敗しました: {type(exc).__name__}: {exc}",
                ephemeral=True,
            )
            return
...
async def send_face_candidate_diagnostics(
    ...
    AI管理側で先にdefer済みでも安全に表示できる。
    メニュー表示時には顔認識エンジンを読み込まない。
    """
    embed = discord.Embed(
        title="🔍 顔候補診断",
        description=(
    ...
    if interaction.response.is_done():
        # AI管理側でdefer済みなので、待機中の元メッセージを完成させる。
